...
exports.handler = async function (argv) {
  ...
  for (const p of points) {
    ...
    if (wallet) {
      ...
      console.error(
        `Could not find network keys for ${patp}: provide them either via wallet or network key file.`,
      );
      process.exit(1);
    }

    var res = await ajs.check.canConfigureKeys(
      ctx.contracts,
      p,
      ethAccount.address,
    );
    ...
    await eth.setGasSignSendAndSaveTransaction(
      ctx,
      tx,
      privateKey,
      argv,
